...
# Retrieve the collector group ID
data "terracurl_request" "get_collector_group" {
  depends_on     = [terracurl_request.get_access_token]
  url            = "${var.vrops_uri}/vrops/suite-api/api/collectorgroups"
  method         = "GET"
  ...
  validation {
    condition = contains(var.vrops_cloud_collector_group_name, data.get_collector_group.CollectorGroups.name)
  }
}
...
# Create the adapter for the VCF instance
resource "terracurl_request" "post_vcf" {
  depends_on     = [terracurl_request.get_access_token]
  url            = "${var.vrops_uri}/vrops/suite-api/api/adapters"
  method         = "POST"
  response_codes = [200, 400, 404, 409, 429, 500]
  headers = {
    Accept        = "application/json"
  ...
  "resourceIdentifiers": [
    {
      ...
      "dataType": "STRING",
      "value": "${var.vcf_server}"
    }
  ],
  "collectorId": ${output.vrops_cloud_collector_id},
  "credential" : {
    "name" : "${var.vcf_server}",
    "adapterKindKey" : "VcfAdapter",
